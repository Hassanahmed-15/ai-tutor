 ...
 *
 * Honesty: this costs a small amount per call and needs OPENAI_API_KEY in apps/web/.env.local.
...
export async function POST(req: Request) {
  if (!process.env.OPENAI_API_KEY) {
    return NextResponse.json(
      { error: "OPENAI_API_KEY not set — add it to apps/web/.env.local to enable the teacher voice." },
      { status: 503 }
    );
  }
