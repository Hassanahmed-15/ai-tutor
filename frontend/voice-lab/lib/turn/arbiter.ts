...
export class TurnArbiter {
  ...
  onFrame(pcm: Float32Array, verdict: DetectorVerdict, now: number): void {
    ...
    if (this.tutor.speaking) {
      ...
      if (who === "student" && sustained >= verifiedNeed) {
        this.commit(now, `verified student voice for ${sustained} ms${this.lastVerdict ? "" : " with no transcript yet"}`);
        return;
      }
      if (replying && who === "unknown" && !this.verifier.enrolled && sustained >= this.config.unverifiedBargeMsReply && !this.lastVerdict) {
        this.commit(now, `unverified voice kept talking over the reply for ${sustained} ms`);
      }
    }
  }
