...
function buildUserMessage(input: LectureBuildInput, retryGuidance: string): string | ContentPart[] {
  const moodLine = input.mood ? `Lesson mode: ${input.mood}. ` : "";
    ...
    try {
      ...
      const minUsableBeats = focusedAnswer
        ...
        : input.focus || fullDocumentPass
          ? (input.focus ? 4 : 8)
          : pdfPlanCount || (input.sourceDocument ? 1 : 8);
...
export async function POST(req: Request) {
   ...
   */
  const job = createJob(session.userId, "Writing the lecture script and boards");
...
async function generateLecture(
  ...
  try {
    ...
    setJobStatus(jobId, "Saving your replayable lecture");
    const archived = await archiveLecture({
      userId,
      topic: input.topic,
      sourceType,
