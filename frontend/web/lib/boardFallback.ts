...
export function hasUsableBoard(beat: Beat): boolean {
  ...
  return ops.some((op) => {
    switch (op.kind) {
      // A placeholder whose fill failed is NOT a board — this is the exact distinction the
      ...
      // Hand-authored ops (labels, shapes, morphs, images) are drawn directly by LiveSketch, so
      // their presence already means the beat has something on screen.
      default:
        return true;
    }
  });
