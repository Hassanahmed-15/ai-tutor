...
function textEl(text: string, x: number, y: number, size: number, color: string, weight: number): string {
  ...
  return `<text x="${gx(x).toFixed(1)}" y="${gy(y).toFixed(1)}" font-size="${size}" font-family="'Comic Sans MS','Trebuchet MS',sans-serif" font-weight="${weight}" fill="${color}" text-anchor="${anchor}">${esc(String(text ?? ""))}</text>`;
}
