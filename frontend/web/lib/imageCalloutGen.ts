...
import { IMAGE_EXPLAINER_SYSTEM_PROMPT } from "./drawPrompt";
...
async function generateOne(
  ...
  try {
    ...
    list.slice(0, 4).forEach((raw, index) => {
      ...
      const text = typeof rec.text === "string" ? stripInlineMath(rec.text.trim()) : "";
      if (!text) return;
      const g = Number(rec.group);
      const group = Number.isFinite(g) ? Math.max(0, Math.min(n - 1, Math.floor(g))) : index;
      const base: CalloutOp = { kind: "callout", text, x: box.x, y: box.y, at: (group + 1) / n };
...
export async function fillImageCalloutOpsIncremental(
  ...
  const pending: Array<{ beat: Beat; beatIndex: number; imageOp: ImageOp; description: string }> = [];
  for (let beatIndex = 0; beatIndex < beats.length; beatIndex++) {
    ...
    pending.push({ beat, beatIndex, imageOp, description });
  }
  ...
  const results = await Promise.all(
    selected.map(async ({ beat, beatIndex, imageOp, description }) => {
      const result = await generateOne(client, beat, imageOp, description);
